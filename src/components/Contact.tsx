        ...
        <div className="grid grid-cols-5 grid-rows-5 gap-0.5 m-0.5 w-full">
            ...
            <div className="row-span-3 col-start-4 row-start-2 bg-white">
                <div className="grid grid-cols-1 grid-rows-5 w-full">
                    ...
                    {/* Loop through the Links const instead of hardcoded */}
                    {links.map((Links, index) => {
                        const Icon = Links.icon;
                        return (
                            <a key={index} href={Links.link} target="_blank" rel="noopener noreferrer">
                                <div className={`${Links.colorBg} flex items-center p-4 h-25 hover:cursor-pointer hover:brightness-75`}>
                                    <Icon className="text-5xl text-white"/>
                                    <label className="text-3xl p-5 text-white hover:cursor-pointer">{Links.name}</label>
                                </div>
                            </a>
                        )
                    })}
